...
def compute_motion_magnitude(tracks: np.ndarray, vis: np.ndarray, track_t: int, track_t_prev: Optional[int] = None) -> float:
    """Mean 3D displacement of mutually visible points from track_t_prev to track_t."""
    if track_t_prev is None:
        track_t_prev = track_t - 1
    if track_t_prev < 0 or track_t >= tracks.shape[0] or track_t_prev >= track_t:
        return 0.0
    visible_both = vis[track_t] & vis[track_t_prev]
    if visible_both.sum() < 1:
        return 0.0
    disp = tracks[track_t, visible_both] - tracks[track_t_prev, visible_both]
